...
int main()
{
    ...
    // position attrbute
    int aPosIdx = 0;
    glVertexAttribPointer(aPosIdx, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(aPosIdx);

    // texture attribute
    int aTexCoordIdx = 1;
    glVertexAttribPointer(aTexCoordIdx, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(3 * sizeof(float)));
